...
if [ $TOOL_PACKAGE == 'jupyterlab' ] || [ $TOOL_PACKAGE == 'notebook' ]; then
    ...
    sh /opt/continuum/scripts/start_zeppelin.sh
elif [[ $TOOL_PACKAGE == 'rstudio' ]]; then
    exec su anaconda -c "/opt/continuum/scripts/start_rstudio.sh"
else
    die "TOOL_PACKAGE ${TOOL_PACKAGE} not in jupyterlab | notebook | anaconda-platform-sync | zeppelin"
fi
